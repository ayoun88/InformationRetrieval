...
# ── [추가] Elasticsearch 인덱스 생성 및 데이터 삽입 함수 ───────────────────────
...
def setup_index(index_name="test", data_path="../data/documents.jsonl"):
    ...
    mappings = {
        "properties": {
            "content": {"type": "text", "analyzer": "nori"}, # 형태소 분석 적용!
            "embeddings": {
                "type": "dense_vector",
                "dims": 768,
                "index": True,
...
def sparse_retrieve(query_str, size=50):
    # content 필드는 이미 nori가 적용되어 있으므로, 
    # query_str이 들어오면 ES 내부에서 알아서 명사 단위로 잘라서 검색합니다.
...
def answer_question(messages):
    ...
    # ── Step 3: [수정] Hybrid 검색 (HyDE 문서를 BM25에도 활용) ──────────────────
    # 질문 원본의 핵심 키워드와 HyDE의 풍부한 단어들을 결합합니다.
    combined_query = standalone_query + " " + hyde_doc

    search_result = hybrid_retrieve(
        sparse_query=combined_query, # BM25에도 HyDE 문서를 함께 넘깁니다!
        dense_query=hyde_doc,        # Dense에는 맥락이 풍부한 HyDE 문서만 넘깁니다.
        size=10
    )
...
if __name__ == "__main__":
    # 1. 인덱스 셋업 실행 (데이터 삽입 및 형태소 분석기 적용)
    # 💡 팁: 한 번 실행 후 다음부터는 아래 줄을 주석(#) 처리하면 매번 임베딩하는 시간을 아낄 수 있습니다.
    setup_index(index_name="test", data_path="../data/documents.jsonl")
    
    # 2. RAG 평가 실행
    eval_rag("../data/eval.jsonl", "v5_index_nori_to_BM25_result.csv")