...
def chunk_text_by_sentences(text, max_tokens=450, overlap_sentences=1):
    """문장 단위로 텍스트를 분할하여 모델의 최대 입력 토큰 수(512)를 넘지 않도록 Chunk 생성"""
    ...
    for doc in raw_docs:
        ...
        if token_len > 512:
            ...
            for i, chunk_text in enumerate(chunks):
                ...
                # 추후 점수 계산을 위해 기존 문서 ID 뒤에 식별용 꼬리표 추가
...
def answer_question(messages):
    ...
    search_result = hybrid_retrieve(
        ...
        size=15 # Reranking을 위해 넉넉하게 후보 추출
    )
    candidates = search_result['hits']['hits']

    # Step 4: Reranking
    reranked_hits = rerank(standalone_query, candidates, top_k=5)
    ...
    for rst in reranked_hits:
        ...
        # [핵심 수정 부분] 채점 시스템이 인식할 수 있도록 "_chunkX" 꼬리표 떼어내기
        base_docid = raw_docid.split("_chunk")[0]

        # 동일한 원본 문서의 여러 청크가 검색되었을 경우, 중복을 제거하고 원본 ID 하나만 topk에 추가
    ...
    # 대회 Base 규격(보통 3개)에 맞춰 topk 슬라이싱
    response["topk"] = response["topk"][:3]

    # Step 5: Answer Generation (Prompt Injection 보완)
...
def eval_rag(eval_filename, output_filename):
    ...
    with open(eval_filename, "r", encoding="utf-8") as f, \
        ...
        for idx, line in enumerate(f):
            ...
            # 베이스라인과 완벽히 동일한 JSONL-in-CSV 제출 규격
            output = {
                "eval_id":          eval_id,
...
if __name__ == "__main__":
    # 1. 인덱스 셋업 실행 (청킹 로직 포함)
    # 한 번 실행하여 인덱싱을 완료한 후에는 이 줄을 주석 처리하세요.
    # setup_index(index_name="test", data_path="../data/documents.jsonl")
    
    # 2. RAG 평가 실행
    eval_rag("../data/eval.jsonl", "v8_512_chunking_result.csv")