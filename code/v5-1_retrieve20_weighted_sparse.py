...
def get_embeddings_in_batches(docs, batch_size=100):
    ...
    for i in range(0, len(docs), batch_size):
        ...
        print(f'  [Embedding] batch {i} ~ {i + len(batch)} 완료')
...
def create_es_index(index, settings, mappings):
    if es.indices.exists(index=index):
        print(f"  [ES] 기존 '{index}' 인덱스를 삭제합니다. (1024차원 안녕!)")
        es.indices.delete(index=index)
    es.indices.create(index=index, settings=settings, mappings=mappings)
    print(f"  [ES] '{index}' 인덱스가 768차원으로 성공적으로 생성되었습니다.")
...
def setup_index(index_name="test", data_path="../data/documents.jsonl"):
    print("\n=== 1단계: Elasticsearch 초기화 및 재색인 시작 ===")
    ...
    print(f"  [ES] 총 {len(docs)}개 문서 임베딩 생성 중... (시간이 조금 걸립니다)")
    ...
    print(f"  [ES] 인덱싱 완벽 종료! (삽입된 문서 수: {ret[0]})")
...
def hybrid_retrieve(sparse_query, dense_query, size=20, rrf_k=60, sparse_weight=1.0, dense_weight=1.0):
    sparse_hits = sparse_retrieve(sparse_query, size=100)['hits']['hits']
    dense_hits  = dense_retrieve(dense_query,   size=100)['hits']['hits']
    ...
    for rank, hit in enumerate(sparse_hits):
        ...
        scores[doc_id] = scores.get(doc_id, 0) + (1.0 / (rrf_k + rank + 1)) * sparse_weight

    for rank, hit in enumerate(dense_hits):
        doc_id = hit['_id']
        docs[doc_id] = hit['_source']
        scores[doc_id] = scores.get(doc_id, 0) + (1.0 / (rrf_k + rank + 1)) * dense_weight
...
if __name__ == "__main__":
    # 🔥 주의: 전체 4,200여 개 문서를 새로 임베딩하므로 약간의 대기 시간이 발생합니다.
    # 인덱싱이 한 번 성공적으로 끝나면, 다음 실행부터는 이 줄을 주석(#) 처리하세요!
    setup_index(index_name="test", data_path="../data/documents.jsonl")
    
    # 평가 실행
    eval_rag("../data/eval.jsonl", "v5-1_retrieve20_weighted_sparse_result.csv")