...
def setup_index(index_name="test", data_path="../data/documents.jsonl"):
    ...
    # Nori 형태소 분석기 설정 — 조사/어미 제거, 명사 중심 토크나이징
    settings = {
        "analysis": {
            "analyzer": {
                "nori": {
    ...
    # [수정] bulk insert 후 명시적 refresh — 이후 검색에서 문서 누락 방지
...
def sparse_retrieve(query_str, size=50):
    # content 필드에 nori 분석기 적용 → 명사 중심 역색인으로 검색
...
def hybrid_retrieve(sparse_query, dense_query, size=10, rrf_k=60):
    ...
    - sparse_query: 원본 standalone_query (BM25는 짧은 키워드 쿼리가 최적)
    - dense_query : HyDE 가상 문서 (의미 유사도 검색에 활용)
    """
...
def answer_question(messages):
    ...
    # ── Step 3: Hybrid 검색 ───────────────────────────────────────────────────
    # [수정] BM25에는 원본 standalone_query만 사용
    #        hyde_doc을 BM25에 섞으면 노이즈 키워드로 점수가 희석됨
    #        Dense 검색에만 hyde_doc을 사용해서 의미 유사도를 극대화
    search_result = hybrid_retrieve(
        sparse_query=standalone_query,  # BM25: 짧고 정확한 키워드 쿼리
        dense_query=hyde_doc,           # Dense: 의미가 풍부한 HyDE 가상 문서
        size=10
    )
...
if __name__ == "__main__":
    # 💡 인덱스가 이미 생성되어 있다면 아래 줄을 주석 처리하면 시간을 아낄 수 있습니다.
    setup_index(index_name="test", data_path="../data/documents.jsonl")

    eval_rag("../data/eval.jsonl", "v6_add_refresh_result.csv")